...
export function ItineraryMap({ stops, tour, focusStopIndex = null }) {
  ...
  const stopsKey = useMemo(
    () =>
      stops?.length
        ? stops.map((s) => `${s.title}|${s.description}|${s.latitude}|${s.longitude}`).join('::')
        : '',
    [stops]
  );
    ...
    const applyMarkers = () => {
      ...
      mapPoints.forEach((point) => {
        ...
        const marker = new mapboxgl.Marker({
          ...
          .setPopup(new mapboxgl.Popup({ offset: 16, closeButton: false }).setText(point.title))
          .addTo(map);
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)]">
      <div
        ref={mapContainerRef}
        className="itinerary-map-container h-[280px] w-full lg:h-[360px]"
      />
      {!mapReady && !mapError && (
