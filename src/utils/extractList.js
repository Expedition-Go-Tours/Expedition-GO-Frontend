...
export function extractList(payload, keys) {
  ...
  for (const key of fallbackKeys) {
    ...
    if (payload[key] && typeof payload[key] === 'object') {
      if (
        payload[key]?.tour &&
        typeof payload[key].tour === 'object' &&
        !Array.isArray(payload[key].tour)
      ) {
        return [payload[key].tour];
      }
      if (
        payload[key]?.tours &&
        typeof payload[key].tours === 'object' &&
        !Array.isArray(payload[key].tours)
      ) {
  ...
  if (payload.tour && typeof payload.tour === 'object' && !Array.isArray(payload.tour))
    return [payload.tour];
  if (
    payload?.data?.tour &&
    typeof payload.data.tour === 'object' &&
    !Array.isArray(payload.data.tour)
  ) {
